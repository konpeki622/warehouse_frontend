        ...
        <el-col :span="12">
          <el-table
            ...
            <el-table-column
              ...
              <template slot-scope="scope">
                <div v-if="scope.row.isEdit">
                  <el-input v-model="scope.row.material_name"></el-input>
                </div>
                <div v-else>
            ...
            <el-table-column
              ...
              <template slot-scope="scope">
                <div v-if="scope.row.isEdit">
                  <el-button size="small" @click="saveEdit(scope.$index)">保存</el-button>
                  <el-button size="small" @click="cancelEdit(scope.$index)">取消</el-button>
                </div>
                <div v-else>
                  <el-button size="small" @click="scope.row.isEdit = true">编辑</el-button>
      ...
      methods: {
        getMaterialData() {
          ...
          this.$http.get(url, {headers: {
            ...
            saveEdit(index) {
                if (this.materialData[index].material_name === '') {
                    this.$message({type: 'error', message: '原料名不能为空!'});
                    return;
                }
                if (this.materialData[index].id === '') {
                    this.$http.post('/apis/material/add?material_name=' + this.materialData[index].material_name, {headers: {
                        'Content-Type': 'application/json',
                        'token': localStorage.getItem("token")
                      }}).then(response => {
                        if (response.status === 200) {
                            this.$message({type: 'success', message: '添加成功!'});
                            this.materialData[index].isEdit = false;
                            this.getMaterialData()
                        }
                        else {
                    ...
                    let url = '/apis/material/update?material_id=' + this.materialData[index].id + '&newName=' + this.materialData[index].material_name;
            ...
            deleteItem(index) {
                this.$confirm('确认删除吗?', '提示', {
                    ...
                    this.$http.post('/apis/material/delete?material_id=' + this.materialData[index].id, {headers: {
                        'Content-Type': 'application/json',
                        'token': localStorage.getItem("token")
                      }}).then(response => {
                        if (response.status === 200) {
